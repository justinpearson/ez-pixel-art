...
Save formats are handled differently. PNG goes through `canvas.toBlob()`. JPG composites onto a white background first, since JPEG has no alpha. BMP is encoded by hand in `encodeBMP24()` — a 24-bit BGR encoder that pre-multiplies alpha against white and writes the BITMAPINFOHEADER manually. SVG is encoded by hand in `encodeSVG()`: it run-length-encodes runs of consecutive same-RGBA pixels into wider/taller `<rect>` elements, runs the pass in BOTH directions (`svgRunsHorizontal` and `svgRunsVertical`), and emits whichever produced fewer rects — so a canvas dominated by 1-pixel vertical features compresses just as well as a row-dominated one. Translucent pixels get a `fill-opacity` attribute; the root `<svg>` carries `shape-rendering="crispEdges"` so rects aren't anti-aliased on render. On the bundled mascot this brought the file from ~11.6k rects / 660 KB down to ~580 rects / 33 KB. If you add a format or change pixel semantics, those encoders need to be updated to match.

SCAD (OpenSCAD) export uses the alpha channel as a depth map: each non-transparent pixel becomes a 1×1×h cube where `h = (alpha / 255) * max_height`. The user supplies `max_height` via the `#scad-depth` input that appears next to `#save-format` only when SCAD is selected (toggled by `syncScadOptionsVisibility`). `encodeSCAD()` mirrors the SVG encoder's structure — it run-length-encodes runs of consecutive same-*alpha* pixels (RGB is ignored; SCAD has no colour) horizontally (`scadRunsHorizontal`) and vertically (`scadRunsVertical`), then emits whichever produced fewer `px()` calls. The Y axis is flipped on the way out so image-row 0 lands at the top in OpenSCAD's +Y-up world. The generated file declares `pixel_size`, `max_height`, a `module px(x, y, w, h, a)` that translates and extrudes a cube, and then one `px(...)` call per run. Different-alpha neighbours are never merged because their cube heights would differ.
...
`prompts/prompt-1.txt` is the original feature request that produced the current app. `prompts/prompt-2.txt` is a list of follow-up feature ideas, most of which are now shipped (alpha channel, brush size, rect/line tools, live cursor preview, in-canvas resize, palette quantizer, all selection modes, recolor selection). `prompts/prompt-3.txt` is the spec for the distinct-colors preview + interactive pixel-sets quantizer + k-means analysis — now shipped. `prompts/prompt-4.txt` is the misc-polish bundle (Draw panel, Eraser tool, Auto-Crop / Crop to Selection, merged selection outline, default startup image, expanded keyboard shortcuts, etc.) — now shipped. `prompts/prompt-5.txt` is a small follow-up tweak list (shortcut re-bindings, save-format selector moved right of Save, Thickness/Filled relocated into the Draw panel, Auto-Crop fixed to trim solid backgrounds, palette swatch grid removed) — now shipped. `prompts/prompt-6.txt` is the SCAD export spec (alpha-as-depth, contextual Depth input shown when SCAD is selected, run-length compression of `px()` calls) — now shipped. Treat the `prompt-*.txt` files as a backlog when planning new work, but per the user's global rules, **never edit any `prompt-*.txt` file** — these are human-authored requirements docs.
